# 👋 Hi, I'm Pooja M M

### AI & Data Science Student | Java Developer | AI/ML Enthusiast

> Building ideas into practical solutions through code, data, and AI.

📍 Coimbatore, India  
🎓 B.Tech — Artificial Intelligence & Data Science  
💡 Interested in Software Development, AI/ML & Cloud

---

### 🚀 What I'm Building

- ☁️ **Cloud Cost Optimizer** — AI-powered cloud cost analysis
- 🎮 **Java Flappy Bird** — Desktop game using Java AWT & Swing
- 📊 **Crime Rate Analysis** — Data analysis & Machine Learning

---

### 🛠️ Tech I Work With

`Java` `Python` `SQL` `ABAP`  
`AI/ML` `AWS` `Git` `GitHub`

---

### 🌱 Currently Learning

`Advanced Java` · `DSA` · `SQL` · `AI/ML` · `Cloud`


### 📊 GitHub

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=poojamanikandan4-commits&show_icons=true&theme=tokyonight&hide_border=true)

---

### 🌐 Connect With Me

[GitHub](https://github.com/poojamanikandan4-commits) •
[Portfolio](https://poojamanikandan4-commits.github.io/) •
[LinkedIn](https://www.linkedin.com/in/pooja-manikandan-9623b6308) •
[LeetCode](https://leetcode.com/u/Poojamanikandan16/)

---

> **Learn → Build → Solve → Improve**
